...
bool USE_PCA = false; // Generate orthogonal matrix by random or by pca.
vector<float> lmds; // All of eigenvalues
vector<float> cdf_lmd; // CDF of first i eigenvalues, i = 1, 2, ..., D
vector<float> epsilon;
float significance = 0.1;

bool FIX_DIM = false;
unsigned int fixed_d = 32;
...
inline float ratio(const int &D, const int &i){
    if(i == D)return 1.0;
        ...
        return cdf_lmd[i] / cdf_lmd[D] * (1.0 + epsilon[i]) * (1.0 + epsilon[i]);
    }else{
...
float dist_comp(const float& dis, const void *data, const void *query, 
    ...
    if (FIX_DIM){
        ...
        if(i && res >= dis * ratio(D, i)){
            ...
            tot_dimension += i;
#endif
            if(USE_PCA) return -res * cdf_lmd[D] / cdf_lmd[i];
            else return -res * D / i;
        }
        float * q = (float *) query;
        float * d = (float *) data;
